...
function marketoSub(evt) {
	...
	ga('send', 'event', 'Sponsorship', window.location.pathname, 'Download Email: ' + this.to.slice(10));
	console.log('eventTrack: ' + 'Download Email: ' + this.to.slice(10) + ' + Marketo Email' );
}

/* Download Email Links in Download Section of Package Builder */
const DownloadEmail = ({ packageName, proposal, openModal, disable }) => {
	...
	return (
			<div className="download-item-wrapper">
				<div className="download-description-wrapper">
					<span className="download-symbol icon-email-big-alternate"></span>
				...
				<div className="download-btns-wrapper">
					...
					<Link 
						to={ (disable.length < 1 ? `/packages/${packageName}` : `/packages/${packageName}/email-proposal`) }  
						className={'button download-button no-max-width ' + (disable.length < 1 ? 'disabled' : 'enabled')} 
						disabled={disable.length < 1}
						onClick={ marketoSub }>
						<span className="icon-email-small"></span>Email</Link>
				</div>
			</div>
		);
}
